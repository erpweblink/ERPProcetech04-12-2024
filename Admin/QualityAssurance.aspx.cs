            ...
            using (con)
            {
                using (SqlCommand cmd = con.CreateCommand())
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblQualityAsurance WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                        string Outward2Qty = "";
                        ...
                        if (row["inwardqty"].ToString() == row["outwardqty"].ToString())
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblQualityAsurance] SET [OutwardQty] = '" + row["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        else
                        {
                            ...
                            SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblQualityAsurance] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + row["SubOA"].ToString() + "'", con);
                            cmdupdate.ExecuteNonQuery();
                        }
                        con.Close();
                    }
