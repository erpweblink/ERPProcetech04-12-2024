...
public partial class Admin_Stock : System.Web.UI.Page
{
                                        ...
                                        using (con)
                                        {
                                            using (SqlCommand cmd = con.CreateCommand())
                                            {
                                                ...
                                                foreach (DataRow roww in dt.Rows)
                                                {
                                                    ...
                                                    SqlCommand cmdexsist = new SqlCommand("select OANumber,InwardQty,OutwardQty,SubOA from [tblDispatch] WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    ...
                                                    SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblStock WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    string Outward2Qty = "";
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblDispatch] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[InwardDtTime]='" + roww["outwarddatetime"].ToString() + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    ...
                                                    if (roww["inwardqty"].ToString() == roww["outwardqty"].ToString())
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblStock] SET [OutwardQty] = '" + roww["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                        cmdupdate.ExecuteNonQuery();
                                                    }
                                                    else
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblStock] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                        cmdupdate.ExecuteNonQuery();
                                                    }

                                                    string OAnum = roww["OAnumber"].ToString();
    ...
    protected void dgvStock_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM [DB_ProcetechERP].[tblUserRoleAuthorization] where UserID = '" + idd + "' AND PageName = 'Stock.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
            ...
            query = @"SELECT * FROM tblStock WHERE  EXISTS ( SELECT 1 FROM [DB_ProcetechTesting].[DB_ProcetechERP].[OrderAcceptDtls] AS OA INNER JOIN [DB_ProcetechTesting].[DB_ProcetechERP].[tblTaxInvoiceDtls] AS TI ON OA.id = TI.OAId  INNER JOIN tblStock AS TS ON TS.SubOA = OA.SubOANumber  WHERE TS.IsComplete IS NULL  AND tblStock.SubOA = TS.SubOA 	AND tblStock.SubOA = '" + Id + @"' ) ORDER BY CONVERT(DateTime, DeliveryDate, 103) ASC;";
    ...
    public void FillGrid()
    {
        try
        {
            ...
            [OutwardDtTime],[OutwardQty],[DeliveryDate],[IsApprove],[IsPending],[IsCancel],[CreatedBy],[CreatedDate],A.[UpdatedBy],
            [UpdatedDate],[IsComplete],CustomerCode FROM tblStock AS A                
			   LEFT JOIN Company AS C ON C.cname=A.customername where IsComplete is null and CustomerName like '" + txtCustomerNameNew.Text.Trim() + "%' order by CONVERT(DateTime, DeliveryDate,103) asc";
    ...
    public void InsertFullReturnQuantity(int TotalReturnInward)
    {
        ...
        string Table = ddlstages.SelectedValue;
        SqlCommand cmdupdate1 = new SqlCommand("SELECT * FROM [tblStock] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        ...
        if (reader.HasRows)
        {
            ...
            if (Success >= 0)
            {
                SqlCommand cmdselect = new SqlCommand("select InwardQty from  tblStock  WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                Object Inwardqty = cmdselect.ExecuteScalar();
                if (Inwardqty == null)
                {
                    SqlCommand cmdDelete = new SqlCommand("DELETE FROM [tblStock] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                ...
                else
                {
                    ...
                    SqlCommand cmdsupdate = new SqlCommand("UPDATE [tblStock] SET [InwardQty] = '" + inquity + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
