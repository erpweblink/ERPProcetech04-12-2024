...
public partial class Admin_PurchaseBillList : System.Web.UI.Page
{
    ...
    private void Gvbind()
    {
        ...
        query = @"select * from tblPurchaseBillHdr AS A
LEFT JOIN tblSupplierMaster AS SM ON SM.SupplierName=A.SupplierName order by A.Id desc";
    ...
    private void GetSupplierDataPopup(string id)
    {
        ...
        query1 = @"select * from tblSupplierMaster where SCode='" + id + "' ";
...
LEFT JOIN tblSupplierMaster AS SM ON SM.SupplierName = A.SupplierName where A.SupplierName like '%" + txtcnamefilter.Text.Trim() + "%' order by A.Id desc";
        ...
        {
            query = @"select * from tblPurchaseBillHdr AS A
LEFT JOIN tblSupplierMaster AS SM ON SM.SupplierName = A.SupplierName where order by A.Id desc";
        }
    ...
    protected void txtSupplierBill_TextChanged(object sender, EventArgs e)
    {
        ...
        {
            query = @"select * from tblPurchaseBillHdr AS A
LEFT JOIN tblSupplierMaster AS SM ON SM.SupplierName = A.SupplierName where A.SupplierBillNo='" + txtSupplierBill.Text.Trim() + "' order by A.Id desc";
        }
        else
        {
            query = @"select * from tblPurchaseBillHdr AS A
