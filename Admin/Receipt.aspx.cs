...
public partial class Admin_Receipt : System.Web.UI.Page
{
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (Session["AgainstVal"].ToString() == "Advance")
                {
                    ...
                    else
                    {
                        ...
                        SqlCommand cmdmax = new SqlCommand("SELECT Max(Recvd)  FROM tblReceiptDtls where InvoiceNo='" + lblinvoiceno.Text + "'", con);
                        con.Open();
            ...
            if (btnsubmit.Text != "Update")
            {
               ...
               SqlCommand cmddtl = new SqlCommand("select SUM(CAST(TaxableAmt as float)) from  tblTaxInvoiceDtls where HeaderID='" + id + "'", con);
                ...
                if (IGSTPer.ToString() == "0")
                {
                    ...
                    decimal CGSTAmt = Amt * Convert.ToDecimal(GSTPer.ToString()) / 100;
                    decimal SGSTAmt = Amt * Convert.ToDecimal(GSTPer.ToString()) / 100;
    ...
    protected void ddltoaccountName_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            txtbankname.Text = "IDFC FIST BANK";
        }
        else
        {
           txtbankname.Text = "Kiran Patel Sir";
        }
    }
