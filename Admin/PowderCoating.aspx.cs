...
public partial class Admin_PowderCoating : System.Web.UI.Page
{
                                    ...
                                    using (con)
                                    {
                                        using (SqlCommand cmd = con.CreateCommand())
                                        {
                                            ...
                                            foreach (DataRow roww in dt.Rows)
                                            {
                                                ...
                                                SqlCommand cmdexsist = new SqlCommand("select OANumber,InwardQty,OutwardQty,SubOA from tblFinalAssembly WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                ...
                                                SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblPowderCoating WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                string Outward2Qty = "";
                                                ...
                                                else
                                                {
                                                    ...
                                                    SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblFinalAssembly] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[IsComplete]=NULL,[InwardDtTime]='" + roww["outwarddatetime"].ToString() + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                ...
                                                if (roww["inwardqty"].ToString() == roww["outwardqty"].ToString())
                                                {
                                                    ...
                                                    SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [OutwardQty] = '" + roww["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    cmdupdate.ExecuteNonQuery();
                                                }
                                                else
                                                {
                                                    ...
                                                    SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblPowderCoating] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    cmdupdate.ExecuteNonQuery();
                                                }
                                                con.Close();
                                            }
    ...
    protected void dgvPowderCoating_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM [DB_ProcetechERP].[tblUserRoleAuthorization] where UserID = '" + idd + "' AND PageName = 'PowderCoating.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
            ...
            gvDetails.DataSource = GetData(string.Format("select * from  [DB_ProcetechERP].[vwPowderCoating] where SubOA='{0}'", Id));
            gvDetails.DataBind();
        }
    }
    ...
    public void InsertFullReturnQuantity(int TotalReturnInward)
    {
        ...
        string Table = ddlstages.SelectedValue;
        SqlCommand cmdupdate1 = new SqlCommand("SELECT * FROM [tblPowderCoating] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
