...
public partial class Admin_SalesQuotationRptPDF : System.Web.UI.Page
{
    ...
    protected void GetQuotationData()
    {
        ...
        QuotationMain.validityofoffer,QuotationMain.deliveryperiod,QuotationMain.specialpackaging,QuotationMain.standardpackaging,QuotationMain.inspection,QuotationMain.transportation,
              Format(QuotationMain.date,'dd-MM-yyyy')as date, QuotationMain.remark, QuotationMain.width, QuotationMain.Toatlamt, QuotationMain.depth, 
              QuotationMain.height, QuotationMain.base, QuotationMain.canopy, QuotationMain.material,QuotationMain.specifymaterial, QuotationMain.Constructiontype,
              QuotationMain.descriptionall, QuotationMain.sessionname, QuotationMain.createddate, QuotationMain.Taxation,QuotationMain.Currency,QuotationData.UOM 
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                var CGStPER = dt.Rows[0]["SGST"].ToString();
                //if (dt.Rows[0]["IGST"].ToString() == "0")
                //{
                ...
                //}
                //if (dt.Rows[0]["IGST"].ToString() == "0" && dt.Rows[0]["CGST"].ToString()=="0")
                //{
                //    lblCGST.Text = "Not Applicable";
                //    lblSGST.Text = "Not Applicable";
                //    lblIGST.Text = "Not Applicable";
    ...
    protected void OnDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Rate += Convert.ToInt32((e.Row.FindControl("lblrate1") as Label).Text);
            var discLabel = e.Row.FindControl("lblDisc") as Label;
            Disc += string.IsNullOrEmpty(discLabel.Text) ? 0 : Convert.ToInt32(discLabel.Text);
            //Disc += Convert.ToInt32((e.Row.FindControl("lblDisc") as Label).Text);
            //Cgstper += Convert.ToInt32((e.Row.FindControl("lblCGSTper") as Label).Text);

            //amount = Rate * Qty-Disc;
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            if (dt.Rows[0]["IGST"].ToString() != "0")
            {
                ...
                lblSGSTamt.Text = cgstamt.ToString();

                //var tot = cgstamt + cgstamt + amount;
                var tot = Igstamt + amount;
                //lblIGST.Text = dt.Rows[0]["IGSTamt"].ToString();
    ...
    private void Pdf()
    {
        ...
        SqlDataAdapter Da = new SqlDataAdapter("SELECT QuotationMain.id,IsRevise,TQ.description, TQ.hsncode, TQ.qty, TQ.rate,TQ.CGST, TQ.SGST, TQ.IGST, TQ.CGSTamt, TQ.SGSTamt, TQ.IGSTamt, TQ.totaltax, TQ.discount, TQ.amount, QuotationMain.quotationno, QuotationMain.partyname," +
            ...
            "Taxation,Currency, TQ.UOM FROM QuotationData TQ INNER JOIN QuotationMain ON TQ.quotationno = QuotationMain.quotationno where QuotationMain.quotationno = '" + ViewState["QuotationNo"].ToString() + "'", con);
        ...
        string imageURL = Server.MapPath("~") + "/img/ProcetechPDFLOGO.jpg";
        ...
        cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "PROCETECH AUTOMATION PVT. LTD.", 180, 745, 0);
        cb.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 8);
        cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "GAT NO 199, BEHIND JYOTIBA TEPMLE JYOTIBANAGAR TALAWADE PUNE,PIMPRI-CHINCHWAD, MAHARASHTRA 411062", 140, 728, 0);
        cb.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 11);
        cb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "", 227, 600, 0);
        cb.EndText();
        ...
        cbbb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "GSTIN : 27AAHCP6642H126", 30, 695, 0);
        cbbb.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 10);
        cbbb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "PAN NO: AAHCP6642H1", 160, 695, 0);
        cbbb.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 10);
        cbbb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "EMAIL :sales@procetechautomation.com", 270, 695, 0);
        cbbb.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 10);
        cbbb.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "CIN: U74900PN2014PTC15", 440, 695, 0);
        cbbb.EndText();
        ...
        if (Dt.Rows.Count > 0)
        {
            ...
            string specialpackaging = Dt.Rows[0]["specialpackaging"].ToString();
            string standardpackaging = Dt.Rows[0]["standardpackaging"].ToString();
            string inspection = Dt.Rows[0]["inspection"].ToString();
            ...
            float[] widths3 = new float[] { 4f, 40f, 8f, 8f, 8f, 0f, 8f, 8f, 10f };
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                table.AddCell(new Phrase("UOM", FontFactory.GetFont("Arial", 10, Font.BOLD)));
            ...
            //GST Calculation
            Double CGStPer = Convert.ToDouble(CGST);
            Double IGStPer = Convert.ToDouble(IGST);
            double GSTamt = Convert.ToDouble(amount) * CGStPer / 100;
            double ISTamt = Convert.ToDouble(amount) * IGStPer / 100;
            string lblCGSTamt = "";
            string lblSGSTamt = "";
            string lblIGSTamt = "";
            ...
            if (Dt.Rows[0]["SGST"].ToString() == "0" && Dt.Rows[0]["IGST"].ToString() == "0")
            {
                lblCGSTamt = "0.00";
                lblSGSTamt = "0.00";
                lblIGSTamt = "Not Applicable";
            ...
            string[] itemss = {
                ...
                "STANDARD PACKING    :    "+ standardpackaging+" \n",
                "SPECIAL PACKING         :    "+ specialpackaging+" \n",
