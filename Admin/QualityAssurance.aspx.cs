...
public partial class Admin_QualityAssurance : System.Web.UI.Page
{
    ...
    {
        SqlDataAdapter ad = new SqlDataAdapter("select FinalAssemblyId, OANumber as OANumber from tblQualityAsurance where IsApprove=1 and IsComplete is null order by FinalAssemblyId desc", con);
        DataTable dt = new DataTable();
        ad.Fill(dt);
        if (dt.Rows.Count > 0)
        {
                                        ...
                                        using (con)
                                        {
                                            using (SqlCommand cmd = con.CreateCommand())
                                            {
                                                ...
                                                foreach (DataRow roww in dt.Rows)
                                                {
                                                    ...
                                                    SqlCommand cmdexsist = new SqlCommand("select OANumber,InwardQty,OutwardQty,SubOA from [tblStock] WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    ...
                                                    SqlCommand cmd2 = new SqlCommand("select OutwardQty from tblQualityAsurance WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    string Outward2Qty = "";
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblStock] SET [InwardQty] = '" + totOutwardqnt.ToString() + "',[IsComplete]=NULL,[InwardDtTime]='" + roww["outwarddatetime"].ToString() + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                    ...
                                                    if (roww["inwardqty"].ToString() == roww["outwardqty"].ToString())
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblQualityAsurance] SET [OutwardQty] = '" + roww["totalinward"].ToString() + "',[InwardQty]='0',[IsComplete]=1,OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                        cmdupdate.ExecuteNonQuery();
                                                    }
                                                    else
                                                    {
                                                        ...
                                                        SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblQualityAsurance] SET [InwardQty] = '" + inwardqy.ToString() + "', [OutwardQty] = '" + totoutward.ToString() + "',OutwardDtTime= '" + OutwardDtTime.ToString() + "',UpdatedDate='" + OutwardDtTime + "' WHERE SubOA='" + roww["SubOA"].ToString() + "'", con);
                                                        cmdupdate.ExecuteNonQuery();
                                                    }
                                                    con.Close();
                                                }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //SqlCommand cmdupdate = new SqlCommand("UPDATE [dbo].[tblQualityAsurance] SET [InwardQty] = '" + TotalReturn_Outward + "',[OutwardQty] = '" + TotalReturn_Outward + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                            //cmdupdate.ExecuteNonQuery();
    ...
    protected void dgvFinalAssembly_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                SqlDataAdapter Sdd = new SqlDataAdapter("Select * FROM [DB_ProcetechERP].[tblUserRoleAuthorization] where UserID = '" + idd + "' AND PageName = 'FinalAssembly.aspx' AND PagesView = '1'", con);
                Sdd.Fill(Dtt);
                if (Dtt.Rows.Count > 0)
                {
    ...
    public void SetFullReturnquntity(int TotalReturnInward)
    {
        ...
        if (Success >= 0)
        {
            SqlCommand cmdDelete = new SqlCommand("Delete from tblQualityAsurance WHERE SubOA='" + hdnSubOANo.Value + "'", con);
            cmdDelete.ExecuteNonQuery();
        }
    ...
    public void InsertFullReturnQuantity(int TotalReturnInward)
    {
        ...
        string Table = ddlstages.SelectedValue;
        SqlCommand cmdupdate1 = new SqlCommand("SELECT * FROM [tblQualityAsurance] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        ...
        if (reader.HasRows)
        {
            ...
            if (Success >= 0)
            {
                SqlCommand cmdselect = new SqlCommand("select InwardQty from  tblQualityAsurance  WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                Object Inwardqty = cmdselect.ExecuteScalar();
                if (Inwardqty == null)
                {
                    SqlCommand cmdDelete = new SqlCommand("DELETE FROM [tblQualityAsurance] WHERE SubOA='" + hdnSubOANo.Value + "'", con);
                ...
                else
                {
                    ...
                    SqlCommand cmdsupdate = new SqlCommand("UPDATE [tblQualityAsurance] SET [InwardQty] = '" + inquity + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
    ...
    {
        string Table = ddlstages.SelectedValue;
        SqlCommand cmdupdate = new SqlCommand("UPDATE  tblQualityAsurance  SET [InwardQty] = '" + TotalReturn_Outward + "',[OutwardQty] = '" + TotalReturn_Outward + "' WHERE SubOA='" + hdnSubOANo.Value + "'", con);
        cmdupdate.ExecuteNonQuery();
