...
public partial class Admin_PurchaseBill : System.Web.UI.Page
{
    ...
    protected void GetPurchaseBillData(string id)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            ddltaxation.SelectedItem.Text = dt.Rows[0]["Taxation"].ToString();
            GstForDirect();
...
protected void btnadd_Click(object sender, EventArgs e)
    {
        ...
        if (ddltaxation.Text != "--Select Type--")
        {
        ...
        else
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Select Tax Type....');", true);
   ...
   // protected void txtTCSPer_SelectedIndexChanged(object sender, EventArgs e)
    //{
       ...
       // {
           // var tot = Convert.ToDecimal(txtGrandTot.Text) + Convert.ToDecimal(txtCost.Text);
            //var tot = Convert.ToDecimal(hdnGrantotalNew.Value) + Convert.ToDecimal(txtCost.Text); //hdnGrandtotal.Value
           // var TcsAmt = Convert.ToDecimal(txtTCSPer.Text) * tot / 100;
            //txtTCSAmt.Text = TcsAmt.ToString("##.00");
   ...
   // }
	  ...
	  protected void txtTCSPer_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            var tot = Convert.ToDecimal(txtGrandTot.Text);
            //var tot = Convert.ToDecimal(hdnGrantotalNew.Value) + Convert.ToDecimal(txtCost.Text); //hdnGrandtotal.Value
            var TcsAmt = Convert.ToDecimal(txtTCSPer.Text) * tot / 100;
            txtTCSAmt.Text = TcsAmt.ToString("##.00");

            var grandtot = Convert.ToDecimal(txtTCSAmt.Text) + Convert.ToDecimal(txtGrandTot.Text);
            txtGrandTot.Text = grandtot.ToString("##.00");
        }
    }
    ...
    private void Show_Grid()
    {
        ...
        hdnGrantotalNew.Value = txtGrandTot.Text;
        FrieghtCalculation();
        TCSCALCULATIONS();
    ...
    protected void dgvParticularsDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (txts == null)
            {
                ...
                //txtGrandTot.Text = (Totalamt + Convert.ToDecimal(txtCost.Text)).ToString();
            }
            else
            {
        ...
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[SP_TaxMaster]", connection))
                {
                    ...
                    if (Dt.Rows.Count > 0)
                    {
                        txtVCGSTPer.Text = Dt.Rows[0]["CGST"].ToString();
                        txtVSGSTPer.Text = Dt.Rows[0]["SGST"].ToString();
                        txtVIGSTPer.Text = Dt.Rows[0]["IGST"].ToString();

                        txtTCGSTPer.Text = Dt.Rows[0]["CGST"].ToString();
                        txtTSGSTPer.Text = Dt.Rows[0]["SGST"].ToString();
                        txtTIGSTPer.Text = Dt.Rows[0]["IGST"].ToString();
        ...
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[SP_TaxMaster]", connection))
                {
                    ...
                    if (Dt.Rows.Count > 0)
                    {
                        CGSTPer.Text = Dt.Rows[0]["CGST"].ToString();
                        SGSTPer.Text = Dt.Rows[0]["SGST"].ToString();
                        IGSTPer.Text = Dt.Rows[0]["IGST"].ToString();
                        txtVCGSTPer.Text = Dt.Rows[0]["CGST"].ToString();
                        txtVSGSTPer.Text = Dt.Rows[0]["SGST"].ToString();
                        txtVIGSTPer.Text = Dt.Rows[0]["IGST"].ToString();
    ...
    public void FrieghtCalculation() 
    {
        String Rate = txtRate.Text;
        if (Rate != "0")
        {
            string Amt = sumofAmount.Text;
            String Cgstper = CGSTPer.Text;
            String Sgstper = SGSTPer.Text;
            String Igstper = IGSTPer.Text;

            if (Cgstper != "0")
            {
                var tot = Convert.ToDecimal(Amt) * Convert.ToDecimal(Rate) / 100;
                var CGSt = Convert.ToDecimal(Cgstper) * tot / 100;
                var SGSt = Convert.ToDecimal(Sgstper) * tot / 100;
                var TotalCost = tot + CGSt + SGSt;
                txtCost.Text = TotalCost.ToString("##.00");
                txtBasic.Text = tot.ToString("##.00");
            }
            else
            {
                var tot = Convert.ToDecimal(Amt) * Convert.ToDecimal(Rate) / 100;
                var Igst = Convert.ToDecimal(Igstper) * tot / 100;
                var TotalCost = tot + Igst;
                txtCost.Text = TotalCost.ToString("##.00");
                txtBasic.Text = tot.ToString("##.00");
            }
        }
