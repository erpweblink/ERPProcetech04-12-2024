...
public partial class Admin_TaxInvoice : System.Web.UI.Page
{
    ...
    protected void LoadData(string id)
    {
        ...
        if (dtload.Rows.Count > 0)
        {
            ...
            txtGSTType.Text = dtload.Rows[0]["TaxType"].ToString();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (txtGSTType.Text != "--Select Type--")
        {
            SaveRecord();
        }
        else
    ...
    public string GenerateCode()
    {
        ...
        {
            maxid = Convert.ToInt32(dt.Rows[0]["maxid"].ToString()) + 1;
        }
        else
        {
            maxid = 1;
        ...
        string strInvoiceNumber = "332425000" + maxid.ToString();
            ...
            if (btnSubmit.Text == "Submit")
            {
                ...
                if (flgs == true)
                {
                    if (!string.IsNullOrEmpty(txtbillingcustomer.Text))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@TransportMode", txttransportMode.Text.ToUpper());
                        cmd.Parameters.AddWithValue("@TaxType", txtGSTType.SelectedItem.Text);
    ...
    protected void gvorder_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (smQty != null && smQty != DBNull.Value)
            {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            TextBox txt_discount = e.Row.FindControl("txtdiscount") as TextBox;
            TextBox txtDiscount = (TextBox)e.Row.FindControl("txtdiscount");
            txtDiscount.Text = "0";
            decimal discount = 0;
            ...
            {
                // Try to parse the value from the TextBox. Use 0 if parsing fails.
                decimal.TryParse(txt_discount.Text, out discount);
            }
    ...
    protected void btnSubmit_Click1(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Warning: Select Tax Type.');", true);
        ...
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[SP_TaxMaster]", connection))
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@TaxCategory", txtGSTType.SelectedItem.Text));
        ...
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[SP_TaxMaster]", connection))
                {
                    ...
                    if (Dt.Rows.Count > 0)
                    {
                        txtGSTType.DataSource = Dt;
                        txtGSTType.DataValueField = "TaxCategory";
                        txtGSTType.DataTextField = "TaxCategory";
                        txtGSTType.DataBind();
                        txtGSTType.Items.Insert(0, "--Select Type--");
    ...
    public void GetPoDetails()
    {
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[SP_PoStatusdetails]", connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@Action", "GetPostatus"));
                    cmd.Parameters.Add(new SqlParameter("@Customer", txtbillingcustomer.Text));
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    DataTable Dt = new DataTable();
                    adapter.Fill(Dt);
                    if (Dt.Rows.Count > 0)
    ...
    public void GetDetailsByPONO()
    {
        try
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@Action", "GetPodetails"));
                    cmd.Parameters.Add(new SqlParameter("@PONo", txtagainstNumber.SelectedItem.Text));
                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    DataTable Dt = new DataTable();
                    adapter.Fill(Dt);
                    if (Dt.Rows.Count > 0)
